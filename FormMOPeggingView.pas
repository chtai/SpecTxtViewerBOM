...
var
  frMOPeggingView: TfrMOPeggingView;
  cdsItem, cdsMO, cdsPO, cdsINV, cdsDO: TclientDataSet;

procedure CallTfrMOPeggingView;
...
begin
  try
    frMOPeggingView := TfrMOPeggingView.Create(application);
    frMOPeggingView.ShowModal;
  finally
    frMOPeggingView.free;
  end;
end;

procedure TfrMOPeggingView.btn_showClick(Sender: TObject);
var
  ...
  begin
    try
      ...
      while not cdsHardpegging.Eof do begin
        sItemid := ''; sTYPE := '';
        sDemandId :=
          cdsHardpegging.FieldByName('SupplyId').AsString + ':' +
          cdsHardpegging.FieldByName('SupplySeq').AsString;

        if cdsHardpegging.FieldByName('SupplyType').AsString = 'M' then begin
          sItemid := ' IssuedMatItemId=' + cdsHardpegging.FieldByName('IssuedMatItemId').AsString + ',' +
            'SupplyItemId=' +cdsHardpegging.FieldByName('SupplyItemId').AsString +
            ',SuuplyQty=' + cdsHardpegging.FieldByName('SupplyQty').AsString;
          sTYPE := '工單';
        end;
        ...
        if cdsHardpegging.FieldByName('SupplyType').AsString = 'P' then begin
          sItemid := ' IssuedMatItemId=' + cdsHardpegging.FieldByName('IssuedMatItemId').AsString + ',' +
           'SupplyItemId=' + cdsHardpegging.FieldByName('SupplyItemId').AsString +
            ',SupplyQty=' + cdsHardpegging.FieldByName('SupplyQty').AsString;
          sTYPE := '採購單';
        end;
        ...
        tmpStrList.Add(sDemandId + ':' + sTYPE + sItemid);
...
begin
  ...
  try
    ...
    ReadTxt('MO_Pegging.txt', cdsHardpegging);
    ...
    for i := 0 to StrList.count - 1 do begin
      cdsDO.Locate('DemandOrderId;Seq',
        ...
        [loPartialKey]);
      sItemid := '';
      if not cdsDO.EOF then
        sItemid := cdsDO.FieldByName('ItemId').AsString
      else
        sItemid := 'VT_Order';
...
procedure TfrMOPeggingView.FormCreate(Sender: TObject);
begin
...
procedure TfrMOPeggingView.btn_ExpClick(Sender: TObject);
begin
